...
hid all of the ``readline`` and ``raw_input()`` functionality under the covers
so we didn't need to worry about it.
...
limits of the ``cmd`` module. The ``cmd`` module is great at one thing:
...
Finally, I came to the conclusion that a new framework would be
required. I took what I had learned from the ``cmd`` module, added in some bash
candy and Python magic, and pypsi was born.

Who should use pypsi?
---------------------

As I stated earlier, I'm not discrediting or degrading the ``cmd`` module, I
